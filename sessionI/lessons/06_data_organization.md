...
author: Mary Piper, Meeta Mistry, Michael Steinbaugh
date: 2017-05-31
duration: 35
---
...
This should give the result: `/home/$USER`.

Now make a directory for the RNA-seq analysis within the `ngs_course` folder using the `mkdir` command. Use the parents flag (`-p` or `--parents`) to create the complete directory structure, as needed.
...
*This is a generic structure and can be tweaked based on personal preferences.*
A brief description of what might be contained within the different sub-directories is provided below:

- `data`: Usually reserved for any raw data files that you start with.
...
```
drwxrwsr-x 2 usr01 usr01   0 Jun 17 11:21 data
drwxrwsr-x 2 usr01 usr01   0 Jun 17 11:21 logs
drwxrwsr-x 2 usr01 usr01   0 Jun 17 11:21 meta
drwxrwsr-x 2 usr01 usr01   0 Jun 17 11:21 results
drwxrwsr-x 2 usr01 usr01   0 Jun 17 11:21 scripts
```
...
**XXX TAKE THIS OUT, RIGHT? NOT DOING TRIMMING?. ALSO, PREFERABLE TO NAME THE FASTQS WITH .FASTQ EXTENSION**
...
**Exercise**: Take a moment to create a README for the `rnaseq` folder (hint: use `vim` to create the file). Give a short description of the project and brief descriptions of the types of files you will be storing within each of the sub-directories. 
