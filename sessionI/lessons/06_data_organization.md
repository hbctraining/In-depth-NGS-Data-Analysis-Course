...
- `data`: Reserved for intermediate, modified data generated by the computational software used for analysis. This can include sequence alignment files (e.g. .bam, .sam) or R binary data (.rda), for example.
- `data-raw`: Store any **unmodified** raw data obtained prior to computational analysis here. We strongly recommend placing the original source NGS run data here, including FASTQ files. This conforms to the [current recommended practice](http://r-pkgs.had.co.nz/data.html) for handling data in R projects.
...
Create a directory for the project by changing into `rnaseq` and then using `mkdir` to create the five directories.
...
Verify that the directories now exist.
...
drwxrwsr-x 2 usr01 usr01   0 Jun 17 11:21 data-raw
