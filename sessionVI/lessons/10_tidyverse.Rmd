...
[dplyr][] underwent a massive revision this year, switching versions from 0.5 to 0.7. If you consult other [dplyr][] tutorials online, note that many materials developed prior to 2017 are no longer correct. In particular, this applies to writing functions with [dplyr][] (see Notes section below).
...
*Note*: `summarize()` also works, for any patriotic Americans o reject British English 🇬.his applies across the board to any tidy functions, including in [ggplot2][] (e.g. `color` in place of `colour`).
